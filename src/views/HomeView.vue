  ...
  <main class="container text-light-text dark:text-dark-text">
    <div class="pt-4 mb-8 relative">
      <input
        ...
        class="py-2 px-1 w-full bg-transparent border-b rounded-md focus:bg-light-card-background dark:focus:bg-dark-card-background focus:outline-none focus:shadow-lg dark:focus:shadow-custom-dark"
      />
      <ul
        class="absolute bg-light-card-background dark:bg-dark-card-background text-light-text dark:text-dark-text w-full shadow-md py-2 px-1 top-[66px] rounded-md text-center"
      ...
      >
        ...
        <p v-if="!searchError && mapboxSearchResults.length === 0">
          Não há resultados que correspondam à sua consulta, tente um termo
          diferente.
        </p>
        <template v-else>
          <li
    ...
    <Suspense>
      ...
      <template #fallback>
        ...
        >
          <svg
            aria-hidden="true"
            class="w-16 h-16 text-gray-200 animate-spin dark:text-gray-600 fill-black dark:fill-white"
            viewBox="0 0 100 101"
            fill="none"
